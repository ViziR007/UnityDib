...
public class MovementControler : MonoBehaviour
{
    ...
    public void Move(float move, bool jump, bool crawling)
    {
        ...
        if (move != 0 && (_grounded || _airControll))
            _player2D.velocity = new Vector2(_speed * move, _player2D.velocity.y); //Движение скорость
        ...
        if (jump && _grounded) // земля проверка касание
        {
        ...
        #region Crawling
        _canStand = !Physics2D.OverlapCircle(_cellCheck.position, _radius, _whatIsGround); //Чтобы не вдавливало в землю при приседании под обьектом

        if (crawling)           // откл колайдер голови
        {
    ...
    void Flip()  //Поворот персонажа на 180 (Туда Сюда- Мальчуген)
    {
